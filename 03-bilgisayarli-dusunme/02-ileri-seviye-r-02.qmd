...
Önce fonksiyon ismini  `toplam` yazdık ve `function` komutu ile, R'a bir fonsksiyon yazacağımızı beyan ettik. Bu fonksiyonun parametreleri olarak kullanacağımız değişkenleri belirledik (`a` ve `b`). 
...
Yazacağımız fonksiyon santigrat cinsinden sıcaklık birimini alsın ve Kelvin'e çevirsin. Fonksiyonun ismi ise `kelvin_donusturucu` olsun.
