...
Bu ders kapsamında ileri seviye R kullanımına başlayacağız. Veri Analizi kısmında daha çok R ile veri analizi üzerine çalışmalar yapıldı. Bu kısımda ise R ile programlama adımlarını incelemeye başlayacağız.

Kontrol yapıları, gerçekleştireceğiniz analiz adımlarında farklı aşamaların çalıştırılması ve veri analizi akışının kontrol edilmesini sağlamaktadır. 
...
Bu vektörde bulunan her elemana farklı işlemler uygulamak istediğimizi düşünelim. R üzerinde kullanacağımız döngü yapıları (`for` döngüsü), bize bu vektördeki her elemana erişip, o elemanla işlem yapma şansı sağlamaktadır.
...
Bunu şu şekild görselleştirebiliriz [@fig-for-loop].
...
Önce bunu bir diyagram ile açıklayalım:

```{dot}
digraph g {
    sayi [label = "Sayı", shape = box];
    kontrol [label = "5'den büyük \n veya \n 5'e eşit", shape = diamond];
    ilk [label = "5'den büyük \n veya \n 5'e eşit", shape = box];
    ikinci [label = "5'den küçük", shape = box];
    sayi -> kontrol;
